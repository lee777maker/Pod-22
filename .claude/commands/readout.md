Produce the block's readout and hand it off. This is the submission — one page, nothing to upload.
...
4. Then the handoff. `readout.html` and `readout-trace.json` are committed by ONE person per block,
   the block's committer, who also publishes the pod's canon:

       python3 pod_sync.py --push-canon

   Everyone else runs `python3 pod_sync.py --take-canon` and commits nothing.
5. The rotation: Build 1's committer is whoever created the pod repo, and it moves down the
   `team/` roster alphabetically from there, one block at a time. If they are not this block's committer, say so
   and give them the `--take-canon` line instead. Two people pushing canon in the same block is the
   conflict this rotation exists to prevent.
